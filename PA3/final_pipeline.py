...
def read_csv(filename):
	'''
	Read a csv and return a data_filefile  
	'''
...
def obtain_basic_statistics(data_file):
	...
	missing_data_file = data_file.iloc[0,:].apply(lambda x : len(data_file) - x)
...
def generate_histogram(data_file):
	...
	for field in data_file.describe().keys():
		...
		pylab.xlabel(field)
		pylab.ylabel("Counts")
		pylab.savefig('histograms/' + field + '.png')
		pylab.close()
...
def go(filename):
    ...
    data_file = data_file.drop("Unnamed: 0",1)
    data_file = generate_features(data_file)
    data_file = clean_data(data_file,FEATURES)
    ...
    letgo = input("Should we continue: Yes or no")

    if letgo == "Yes":

	    best_overall_model, best_overall_params = model_loop(MODELS, 
	    	data_file, LABEL, N_FOLDS)
	    
	  
	    train, test = train_test_split(data_file, test_size = 0.2)
	    evaluate_final_model(train,test,best_overall_model,best_overall_params)
...
if __name__=="__main__":
    ...
    with open("models_comparison.csv", 'w') as f:
        w = csv.writer(f,delimiter = ",")
        w.writerow(["MODEL","AVERAGE","SD","COUNT"])
        ...
        for classifier in MODELS:
            ...
            for key,values in d.items():
                if values["Model"] == classifier:
                    count += 1
                    class_mean += values["Average AUC"]
                    class_sd += values["SD AUC"]

            w.writerow([classifier,class_mean,class_sd,count])
